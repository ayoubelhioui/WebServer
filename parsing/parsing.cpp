...
void errorsChecking(std::list<Parsing> &parsingData)
{
...
void readingData(std::list<Parsing> &parsingData){
    ...
    while (std::getline(configFile, enteredData))
        fileData.push_back(enteredData);
    std::list<std::string>::iterator it = fileData.begin();
    while (it != fileData.end())
    {
        Parsing parsingNode;
        while ((*it).find("location") == std::string::npos)
            parsingNode.fillingDataFirstPart(*it++);
        while((*it).find("location") != std::string::npos)
        {
            locationBlock loc;
            location_parse(it, loc);
            if((*it).find("location") == std::string::npos
            && *it != "}") {
                errorPrinting("error : You didn't close the server or may have other problems");
            }
            parsingNode.Locations.push_back(loc);
        }
        parsingData.push_back(parsingNode);
        if (*it == "}") it++;
//           break ;
    }
//    printingParsingData(parsingData);
